...
export function Print({ content, children, style }: { content?: ReactNode; children: ReactElement; style?: StyleString }) {
...
function Content({
  ...
  const handlePrint = () => {
    invariant(printRootRef.current?.contentWindow, "<Frame /> missing `contentWindow`");
  ...
  return (
    <Frame initialContent={getInitialContent(style)} ref={printRootRef}>
      {children}
    </Frame>
  );
}
